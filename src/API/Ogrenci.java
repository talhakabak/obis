...
public class Ogrenci {
    ...
    public static void ogrenciBilgi(String online) { //Giriş yapan öğrencinin bilgilerini alır
        ...
        for (int i = 0; i < Database.ogrenciler.length && f == (-1); i++) {//sırayla dizi içerisindeki dizilerde değer aratır
            h++;//
            f = Arrays.asList(Database.ogrenciler[i]).indexOf(online);//dizinin kaçıncı elemanı ise o değeri alır. almazsa -1 kalır. alana kadar devam eder
            ogrenciOnline = Database.ogrenciler[h];
        }
    }
